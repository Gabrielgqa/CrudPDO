...
class Usuario
{
    ...
    public function insert($pdo){
        $sth = $pdo->prepare("INSERT INTO usuarios(nome,email,senha,tipo,setor,ativo) VALUES (:nome,:email,:senha,:tipo,:setor,:ativo)");
    ...
    public function update($id, $pdo){
        $sth = $pdo->prepare("UPDATE usuarios SET nome=:nome, email=:email, senha=:senha, tipo=:tipo, setor=:setir WHERE id=:id LIMIT 1");
        $sth->BindValue(':nome',$this->nome,PDO::PARAM_STR);
        $sth->BindValue(':email',$this->email,PDO::PARAM_STR);
        $sth->BindValue(':senha',$this->senha,PDO::PARAM_STR);
        $sth->BindValue(':tipo',$this->tipo,PDO::PARAM_INT);
    ...
    public static function selectAll($pdo){
        $sth = $pdo->query("SELECT * FROM usuarios ORDER BY nome ASC");
        $sth->execute();
    ...
    public static function mudaAtivo($ativo, $id,$pdo){
        $sth = $pdo->prepare("UPDATE usuarios SET ativo=:ativo WHERE id=:id LIMIT 1");
